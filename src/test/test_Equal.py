...
class EqualValidatorTests(unittest.TestCase):
    ...
    #     validator =TestValidator(lambda v: v.RuleFor(lambda x: x.Forename).Equal(lambda x: x.Surname))
    ...
    def test_Validates_against_property(self):
        validator = TestValidator(lambda v: v.RuleFor(lambda x: x.Surname).Equal(lambda x: x.Forename).WithMessage("{ComparisonProperty}"))
    ...
    # 	validator =TestValidator(lambda v: v.RuleFor(lambda x: x.Surname).Equal(lambda x: x.Forename, StringComparer.OrdinalIgnoreCase))
