...
class GreaterThanValidatorTester(unittest.TestCase):
	...
	def __init__(self, *args, **kwargs):
		...
		self.validator = TestValidator(lambda v: v.RuleFor(lambda x: x.Id).GreaterThan(self.VALUE))
	...
	def test_validates_with_nullable_property(self)-> None:
		self.validator = TestValidator(lambda v: v.RuleFor(lambda x: x.Id).GreaterThan(lambda x: x.NullableInt))
	...
	def test_Validates_nullable_with_nullable_property(self)-> None:
		validator = TestValidator(lambda v: v.RuleFor(lambda x: x.NullableInt).GreaterThan(lambda x: x.OtherNullableInt))
	...
	def test_Validates_with_nullable_when_property_is_null(self)-> None:
		validator = TestValidator(lambda v: v.RuleFor(lambda x: x.NullableInt).GreaterThan(5))
	...
	def test_Validates_with_nullable_when_property_is_null_cross_property(self)-> None:
		validator = TestValidator(lambda v: v.RuleFor(lambda x: x.NullableInt).GreaterThan(lambda x: x.Id))
		result = validator.validate(Person(Id = 5 ))
		self.assertTrue(result.is_valid)


	def test_Validates_with_nullable_when_property_not_null_cross_property(self)-> None:
		validator = TestValidator(lambda v: v.RuleFor(lambda x: x.NullableInt).GreaterThan(lambda x: x.Id))
